...
stg_dict = {
    ...
    'ladder_fibonachi_stg':
        {
            'stg_name': 'ladder_fibonachi_stg',
            'name': 'Лесенка фибоначи',
            'desc': 'Стратегия торговли лесенкой с фибоначи',
            'step': '0.02',
            'amount': 1,
            'deals': 1,
            'ctg': 'linear',
...
class Strategy_Step_Fibonachi(Api_Trade_Method):
    symbol: str
    decimal_part: int
    uuid: str
    session: None
    ...
    def __init__(self, stg_id, user_id):
        ...
        self.api_session = self.makeSession(stg_id=stg_id)
        self.user_id = user_id
        self.stg_name = 'ladder_fibonachi_stg'
        self.stg_dict = self.getStgDictFromBD()
        config.getTgId(user_id=user_id)
        ...
        if tradeQ.first():
            ...
            if tradeQ and self.stg_dict['deals'] >= tradeQ.count() or lastTX.price == str(lastPrice):
                ...
                if 'error' not in tx:
                    ...
                    tx['result']['tpOrderId'] = tp['result']['list'][0]['orderId']
                    self.createTX(tx=tx)
                    config.message = f"[" + emoji.emojize(":check_mark_button:") + f"] Куплено повторно по {lastPrice}"
                    config.update_message = True
        else:
            tx = self.BuyMarket(self.symbol, self.stg_dict['amount'])
            if 'error' not in tx:
